...
bool ChessBoard::isMovable(const Coord &aftercod, const Coord &beforecod, const int player)
{
	...
	if (player == 0) // PLAYER_BLACK
	{
		...
		case 1: // black general
			...
			else if (before.x == after.x && after.y >= 7 && after.y <= 9)
			{
				for (int hasgeneral = 0, i = before.y + 1; i <= 9; i++)
				{
					...
					if (i == 9 && hasgeneral == 1 && area[after.y][after.x].getKind() == 8)
					{
						return true;
					}
				}
	...
	else if (player == 1) // PLAYER_RED
	{
		...
		case 8: // red general
			...
			else if (before.x == after.x && after.y >= 0 && after.y <= 2)
			{
				for (int hasgeneral = 0, i = before.y - 1; i >= 0; i--)
				{
					...
					if (i == 0 && hasgeneral == 1 && area[after.y][after.x].getKind() == 1)
					{
						return true;
					}
				}
...
bool ChessBoard::moveChess(const Coord &currLoc, const Coord &cursorLoc, const int player)
{
	if (isMovable(cursorLoc, currLoc, player))
	{
		// replace the chess on cursor loc
		area[cursorLoc.y][cursorLoc.x] = area[currLoc.y][currLoc.x];

		// clear current loc
		Chess cleaner;
		area[currLoc.y][currLoc.x] = cleaner;
